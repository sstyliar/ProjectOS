...
void next_level(human h1, monster m, boss b, char **temp_maze, int width, bool defeated, char *maze, int sock);

// gameStatsInit
human humanInit(human h){
   
   strcpy(h.stats.username, "Username Bug.");
   h.stats.password = 0;
   h.stats.health = 0;
   h.stats.armor = 0;
   h.stats.attack = 0;
...
//CLIENT SIDE
int monster_Attack_human(WINDOW *win, human *h1,monster m, boss b, int health){  
   int damage;   
   // int health = h1->stats.health;                               
   int rnd = rand()%100;      
   float hit = rnd/100.0;
   ...
   if(hit <= m.stats.accuracy){
      if(damage >= 0 ){
         ...
         if (health >= 0)
         {
            ...
            mvwprintw(win, 3, 15, "  ");
            mvwprintw(win, 3, 1, "Human health: %d",health);
...
//CLIENT SIDE
int boss_Attack_human(WINDOW *win, human *h1,monster m, boss b, int health){  
   int damage;   
   // int health = h1->stats.health;                             
   int rnd = rand()%100;      
   float hit = rnd/100.0;
   ...
   switch(swit){
      case 1:
        do{
         time_t t;
            ...
            health = monster_Attack_human(win, h1,m,b, health);
            sleep(1);
            if(health==0 || m.stats.health==0){
               if(m.stats.health == 0){
                  winner = 1;
                  fighting = FALSE;       
               }
      ...
      case 2:
         do{
            ...
            health = boss_Attack_human(win,h1,m,b, health);
            sleep(1);
            if(health==0 || b.stats.health==0){
               if(b.stats.health == 0){
                  winner = 1;
                  fighting = FALSE;       
               }
...
bool assignPoints(WINDOW *window, WINDOW *target, WINDOW *stats, char **temp_maze, human* h1, int direction, int maxX, int width, bool start_game, bool nextLevel, int sock, char *maze){
   ...
   //If this costs us the 5 i will become a pirate and sell low quality dvds for a living.
   if (start_game == TRUE)
   {
      mvwprintw(target,1,1,"Use the arrow keys to move around.");   
      mvwprintw(target,2,1,"If you want to exit the game press the End key.");   
      mvwprintw(target,3,2,"Please press 'y' to agree to the Terms and Conditions.");   
      mvwprintw(target,4,2,"Or press 'n' if you disagree.");   
      wrefresh(target);
      h1->loot.points = 50;
   }
   else{
   ...
   switch(ch)
   {
      case 'y':  
            ...
            switch(direction){
               case KEY_UP:
                  ...
                  maze[((h1->pos.y-1)*width)+(h1->pos.x/4)]=0;
                  thinking = FALSE;
                  break;
               case KEY_DOWN:
                  clearWindow(target, maxX);
               ...
               case KEY_RIGHT:
                  ...
                  maze[((h1->pos.y)*width)+(h1->pos.x/4)+1]=0;
                  thinking = FALSE;
                  break;
               case KEY_LEFT:
                  clearWindow(target, maxX);
      ...
      case 'n':
         clearWindow(target, maxX);
         mvwprintw(target,1,1,"No such choice, please try again.");  
         wrefresh(target);
         sleep(2);
         break;
   }
...
bool userControl(WINDOW *window, WINDOW *target, WINDOW *stats,WINDOW *Score,char *maze, char **temp_maze, bool thinking, bool bossEncounter,
               human *h1, monster m, boss b, int direction, int maxX, int width, int sock){
   ...
   switch(ch)
   {
      case 'y':
         ...
         if(bossEncounter == FALSE){
            ...
            clearWindow(target, maxX);
            mvwprintw(target,1,1,"Fighting ended and...");
            sleep(1);  
            if (winner == 1)
            {
               mvwprintw(target,2,1,"Winner was declared the Human."); 
               mvwprintw(target, 3, 1, "The path is free.");
               h1->loot.points += 10;
               mvwprintw(stats, 7, 7, "  "); 
               mvwprintw(stats, 7, 7, "%d",h1->loot.points); 
               switch(direction){
            ...
            mvwprintw(stats, 2, 9, "0 ");    //Zero humam's health
            wrefresh(target);
            wrefresh(stats);
            wrefresh(Score);
            sleep(3);
         ...
         if (bossEncounter == FALSE){
            switch(direction){
               case KEY_UP:
                  ...
                  mvwprintw(target, 1, 1, "The path is free.");
                  wrefresh(window);
                  wrefresh(target);
                  temp_maze[h1->pos.y-1][h1->pos.x] = 0;
                  temp_maze[h1->pos.y-1][h1->pos.x+1] = 0;
                  ...
                  maze[((h1->pos.y-1)*width)+(h1->pos.x/4)]=0;
                  maze[((h1->pos.y-2)*width)+(h1->pos.x/4)]=0;
                  thinking = FALSE;
                  break;
               case KEY_DOWN:
                  clearWindow(target, maxX);
               ...
               case KEY_RIGHT:
                  ...
                  thinking = FALSE;
                  maze[((h1->pos.y)*width)+(h1->pos.x/4)+1]=0;
                  maze[((h1->pos.y)*width)+(h1->pos.x/4)+2]=0;
            ...
            clearWindow(target, maxX);
            mvwprintw(target, 1, 1, "Oy mama, can't send boss away.");
            wrefresh(target);
            thinking = FALSE;
            break;
         }
...
void mainController(char *maze, int width, int height, char **temp_maze, human h1, monster m, boss b,
   ...
   {
      while(thinking){
         thinking = assignPoints(Maze,Output,Stats, temp_maze, &h1, ch, maxX, width, start_game,TRUE,sock, maze);
      }
   }
   ...
   switch (ch){
      case KEY_UP:
         if (h1.pos.y == 0){
               mvwprintw(Output,1, 1, "You can't go up, go down...please!");
               wrefresh(Output);
               break;
               ...
               clearWindow(Output,maxX);
               mvwprintw(Output,1, 1, "You hit wall.");
               wrefresh(Output);
               break;
         }else if(temp_maze[h1.pos.y-1][h1.pos.x] == 3){
               clearWindow(Output,maxX);
               mvwprintw(Output,1,1,"You found rare monster Vasilw.");
               ...
               clearWindow(Output,maxX);
               mvwprintw(Output,1,1,"Found Final Boss.");
            ...
            wrefresh(Output);
            while(thinking){
               thinking = assignPoints(Maze, Output, Stats, temp_maze, &h1, ch, maxX, width, FALSE, FALSE,sock,maze);
            }
            break;
         }
      case KEY_DOWN:
      ...
      case KEY_LEFT: 
         if (temp_maze[h1.pos.y][h1.pos.x - 2] == 0)
         {
            ...
            clearWindow(Output,maxX);
            mvwprintw(Output,1,1,"You found rare monster.");
            mvwprintw(Output,2,1,"Do you wish to fight and be rewarded upon victory?");
            mvwprintw(Output,3,1,"Press 'y' to start fighting or 'n' to decline or 'r' to run.");
            mvwprintw(Output,4,1,"If you decline Vasilw will leave with the money.");
            wrefresh(Output);
